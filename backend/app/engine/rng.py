...
All helpers are sync and import only hashlib/random/typing — no FastAPI/etc.
"""
...
def derive_seed(
    ...
    ``"{run_seed}|{ruleset_version}|{turn}|{namespace}|{entity_id}|{ordinal}"``
    encoded as utf-8. Uses ``hashlib.blake2b(digest_size=8)``.
    ...
    """
    canonical = f"{run_seed}|{ruleset_version}|{turn}|{namespace}|{entity_id}|{ordinal}"
