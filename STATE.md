...
backend/
  app/
    ...
    engine/
      ...
      actor.py               # NEW shared primitives: YIELD_PER_CAPACITY/DROUGHT_BPS/COSTS + compute_farm_output/resolve_buy/resolve_storage_settlement/resolve_shipment/resolve_expand_farm/resolve_build_granary/resolve_secure_route/value_for/cost_for_quantity (single source, integer, re-exported by turn.py)
      ...
      rivals.py              # NEW deterministic rivals: RivalProfile(farm-averse/hungry prefs bps)/RivalState(cash/inventory/farm/storage/route, no headline)/RivalTurnResult(before/command/after/headline/reason_code/cash/qty/price/wealth deltas)/ObservableContext/SettlementContext + integer/bps scoring (expected_return×pref×capital×risk×exposure, threat boost T3→drought, exact-tie rng_for) + apply_rival_command via actor primitives + truthful headlines via (reason_code,buy_actual,ship_effective); MIRA_START 1200/25/5/250/DARAN 1400/15/12/150
  ...
  tests/
    ...
    test_deterministic_rivals.py # NEW Section 7 AC1-6+partials: profiles differ identical (≥2 diffs Mira storage> Daran Daran farm> Mira), determinism same seed, capital via shared primitives expand/build/secure/buy/ship (zero-cash hold, no negatives, cost parity, buy cap, ship cap), no-money price reval, behavioral prep vs concentration (Mira prep > baseline +40% on warning Mira prep>farm Daran farm>prep), fingerprint 5 contexts, headlines derived via (reason_code,actual qty) & visible after first (len5 T2+ non-empty, truthful expand/buy/ship with resolved deltas, partial buy 10→4 with insufficient_cash reports accumulated not "could not buy", partial ship 10→7 with insufficient_cash_for_transport reports shipped not "could not ship"), execution same rules, market isolation, no float, structured threat not prose, RivalState no headline, exact-tie rng
...
- Canonical state frozen with immutable tuples; `RivalState` frozen `ge=0` without headline; `RivalProfile` frozen prefs bps; `RivalTurnResult` frozen with `reason_code` + full deltas and wealth `cash+qty+price` (price revaluation at `home_resolved`), headline derived from `(reason_code, buy_actual, ship_effective)` so partial `insufficient_cash`*→*buy and `insufficient_cash_for_transport`→ship are truthful; rival wealth never floats.
...
make test              # = uv run --project backend pytest -v  (112 passed: 96 prior +16)
...
pytest -v                  → 112 passed (96 prior +16) (8 core +7 rounding+11 determinism+2 purity/sanity+15 kernel+7 invariants+12 causal_trace+4 explanation+13 two_markets_route+17 five_turn_prototype+16 deterministic_rivals [profiles differ identical, determinism with divergence, capital via shared primitives expand/build/secure/buy/ship, no-money with revaluation incl. price, behavioral warning prep vs farm, fingerprint, headlines derived via (reason_code,buy_actual,ship_effective) — insufficient_cash→buy vs "could not buy" and insufficient_cash_for_transport→ship vs "could not ship" for partials — plus insufficient_storage/already_established/insufficient_inventory vs no_route_access, same-rules, market isolation, structured threat via next_world_known, canonical 5/5 diffs, no float, RivalState Money/Quantity strict, RivalPreferences frozen, exact-tie rng, partial regressions])
