...
## Corpus Check
- 78 files · ~123,458 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1142 nodes · 1969 edges · 90 communities (80 shown, 10 thin omitted)
- Extraction: 96% EXTRACTED · 4% INFERRED · 0% AMBIGUOUS · INFERRED: 74 edges (avg confidence: 0.51)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `d304155d`
...
Cohesion: 0.08
Nodes (48): format_market_pulse(), format_player_state(), format_rivals(), format_route_state(), main(), parse_choice(), parse_choices_arg(), Thin CLI for the five-turn headless prototype — Section 6. Pure I/O around… (+40 more)

### Community 3 - "test_five_turn_prototype.py"
Cohesion: 0.10
Nodes (28): default_start_state(), Tuned start state — Section 9 retuned for binding constraints (storage…, Regression: starting storage must be scarce enough that granary matters. Idle…, test_build_granary_not_worthless(), _choices(), _hold(), Section 6 — Five-Turn Headless Prototype acceptance tests. AC1: exactly 5…, AC5 non-dominance: no one policy wins every meaningful outcome. Hold may win… (+20 more)
...
Cohesion: 0.05
Nodes (66): Domain package — re-exports canonical types., Pressure domain — Section 8 world-pressure arc types. Small, frozen, validated…, CausalNode, CausalTrace, DomainEffect, OutcomeDriver, PlayerOutcome, BaseModel (+58 more)
...
### Community 71 - "SettlementContext"
Cohesion: 0.25
Nodes (8): Economic settlement context post player market resolution., SettlementContext, Buy/ship/expand costs and clamping use same actor primitives., Partial buy (cash limits 10→4) must not say 'could not buy' — headline reflects…, Partial ship (transport cash limits 10→7) must not say 'could not ship'., test_partial_buy_insufficient_cash_headline_truthful(), test_partial_ship_insufficient_cash_for_transport_headline_truthful(), test_rival_execution_same_rules_as_player()
...
### Community 76 - "RivalState"
Cohesion: 0.20
Nodes (10): cost_for_quantity(), Cost in Money for quantity at price_milli (milliunits per unit). Floor division…, _capital_bps(), _expected_return(), Rough integer expected return in Money for scoring (not canonical wealth).…, Capital factor in bps: 0 if unaffordable, 5000 if tight, 10000 if comfortable., Risk factor: penalize if post-command cash would be low., Economic state — cash/inventory/capacities/route, no headline. (+2 more)
...
Cohesion: 0.12
Nodes (27): affordable_quantity(), compute_farm_output(), Shared actor-level economic primitives — Section 7 extraction. Single source of…, Resolve a sell_grain command — inverse of buy, at Home price. Args: cash: cash…, Settle harvest into inventory with storage cap. Returns…, Resolve ship_grain settlement with shared clamping. Uses pre-turn…, Shared expand_farm affordability/mutation. Returns (cash_after, farm_after,…, Shared build_granary affordability/mutation. Returns (cash_after,… (+19 more)

### Community 80 - "FiveTurnGame"
Cohesion: 0.11
Nodes (10): FiveTurnGame, In-memory 5-turn game — owns GameState, history, and session rivals. Rivals are…, Derived headlines per turn for UI convenience., Build pre-turn observable context for rival choice (structured, not prose)., Rivals must not mutate shared market availability — player supply same as…, Canonical hold-5 run must have ≥1 turn where Mira/Daran choose different types…, test_canonical_run_has_three_diffs(), test_headlines_derived_and_visible_after_first_turn() (+2 more)
...
Cohesion: 0.19
Nodes (10): BaseModel, Five-turn headless prototype — Section 8 pressure-driven arc. Orchestrates…, Wealth = cash + inventory value at Home price (milli)., Concise end-of-run summary — both human and structured (now with rivals)., One turn's authored world and signal — plain strings, not a DSL., StrategicSummary, TurnSpec, _wealth() (+2 more)

### Community 86 - "RivalProfile"
Cohesion: 0.22
...
### Community 90 - "InventoryState"
Cohesion: 0.16
Nodes (25): InventoryState, Player grain inventory — single good grain for Sections 2-4., choose_rival_command(), ObservableContext, Information visible to rivals when choosing (pre-turn)., Integer/bps score for a given rival + command + observable context. score =…, Deterministically choose rival command via integer scoring. Rankings by…, score_rival_command() (+17 more)
...
## Knowledge Gaps
- **543 isolated node(s):** `B1 — BLOCKING: the plan makes the balance harness's scripted policies into the player's only legal moves`, `B2 — The causal trace must not be behind a debug flag or a query param`, `B3 — The AC4 test has one vacuous step and one circular step`, `B4 — `empire_summary.operations` is invented structure`, `B5 — Internal contradiction: `run_seed` is required for reproduction but absent from `GameView`` (+538 more)
...
- **Why does `PlayerCommand` connect `PlayerCommand` to `test_pressure_arc.py`, `harness.py`, `test_five_turn_prototype.py`, `GameState`, `SettlementContext`, `RivalState`, `rivals.py`, `FiveTurnGame`, `prototype.py`, `RivalProfile`, `InventoryState`?**
  _High betweenness centrality (0.042) - this node is a cross-community bridge._
- **Why does `FiveTurnGame` connect `FiveTurnGame` to `test_pressure_arc.py`, `harness.py`, `test_five_turn_prototype.py`, `GameState`, `SettlementContext`, `PlayerCommand`, `RivalState`, `rivals.py`, `prototype.py`, `InventoryState`?**
  _High betweenness centrality (0.033) - this node is a cross-community bridge._
- **Why does `resolve_turn()` connect `PlayerCommand` to `test_pressure_arc.py`, `harness.py`, `test_determinism.py`, `GameState`, `test_rounding.py`, `test_five_turn_prototype.py`, `rivals.py`, `FiveTurnGame`, `prototype.py`, `InventoryState`?**
