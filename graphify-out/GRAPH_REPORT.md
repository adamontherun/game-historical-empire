...
## Corpus Check
- 91 files · ~140,435 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1303 nodes · 2320 edges · 100 communities (89 shown, 11 thin omitted)
...
Cohesion: 0.08
Nodes (28): PressureState, BaseModel, model_validator, One step of the authored pressure arc — frozen, validated. Exactly 7 fields per…, Engine package — deterministic RNG and rounding., next_world_known_for_turn(), pressure_for_turn(), Pressure arc — Section 8 authored 5-turn arc. Hard-coded, deterministic, no… (+20 more)

### Community 2 - "Part I — Product North Star"
Cohesion: 0.50
...
Cohesion: 0.09
Nodes (55): choice_map_for(), choices_for(), _completion_summary(), _market_view(), _outcome_view(), Mappers: FiveTurnGame -> GameView. Pure, no mutation., Map choice_id -> PlayerCommand for current revision., Legality + affordability ONLY (B1). No turn gating, no margin>0 gating. Two… (+47 more)
...
Cohesion: 0.13
Nodes (14): 23. Systems Explicitly Deferred Until Proven Necessary, 29. Player Experience Success Criteria, 30. Technical Success Criteria, 31. Do Not Build Ahead, 32. Do Not Rewrite Working Systems Without Evidence, 33. Dependencies Require Justification, 34. Agent Completion Report Template, 35. First Active Section (+6 more)
...
Cohesion: 0.08
Nodes (23): 001 — Scope Strategy (2026-08-09), 002 — FastAPI Conventions (2026-08-09), 003 — Tooling (2026-08-09), 004 — Testing Strategy (2026-08-09), 005 — Frontend Stack (2026-08-09), 006 — Persistence (2026-08-09), 007 — Autonomy (2026-08-09), 008 — Plan Mode (2026-08-09) — revised 2026-08-09 (+15 more)
...
Cohesion: 0.08
Nodes (66): InventoryState, Player grain inventory — single good grain for Sections 2-4., compute_farm_output(), cost_for_quantity(), Resolve a sell_grain command — inverse of buy, at Home price. Args: cash: cash…, Cost in Money for quantity at price_milli (milliunits per unit). Floor division…, Inventory value in Money at price_milli., Compute farm output for given capacity and world. Returns (farm_output,… (+58 more)
...
Cohesion: 0.18
Nodes (11): Domain package — re-exports canonical types., CausalNode, PlayerOutcome, Causal trace, domain effects, and player outcome for Sections 4–5. Structural…, Concise player outcome for the turn reveal., Full result of resolving one turn., One step in the causal chain with explicit parent links., TurnResolution (+3 more)

### Community 91 - "turn.py"
Cohesion: 0.16
Nodes (16): affordable_quantity(), Shared actor-level economic primitives — Section 7 extraction. Single source of…, Settle harvest into inventory with storage cap. Returns…, Resolve ship_grain settlement with shared clamping. Uses pre-turn…, Shared expand_farm affordability/mutation. Returns (cash_after, farm_after,…, Shared build_granary affordability/mutation. Returns (cash_after,…, Shared secure_route affordability/mutation. Returns (cash_after, route_after,…, Max quantity affordable at price_milli with cash, floored cost. (+8 more)
...
Cohesion: 0.17
Nodes (22): GameState, MarketState, OperationState, PlayerState, BaseModel, Top-level canonical game state for Sections 2-5. For backward compatibility,…, Economic operation — stub for farm/granary, extensible later. Represents a farm…, Player economic exposure — single source of truth for capacities. For Sections… (+14 more)

### Community 97 - "harness.py"
Cohesion: 0.15
Nodes (18): BatchResult, _can_afford(), format_markdown(), policy_production_heavy(), policy_random_legal(), policy_storage_heavy(), policy_trade_heavy(), _policy_trade_heavy_no_route() (+10 more)

### Community 98 - "prototype.py"
Cohesion: 0.14
Nodes (13): In-memory game sessions — Section 10. Per-session asyncio.Lock so GET and POST…, Pressure domain — Section 8 world-pressure arc types. Small, frozen, validated…, Canonical economic types for Sections 2-5. Integer-only canonical state per…, River Route — single route between Home Valley and River Town (Section 5).…, RouteState, BaseModel, Five-turn headless prototype — Section 8 pressure-driven arc. Orchestrates…, Wealth = cash + inventory value at Home price (milli). (+5 more)
...
## Knowledge Gaps
- **611 isolated node(s):** `historical-empire-backend`, `Usage`, `What graphify is for`, `Step 0 - GitHub repos and multi-path merge (only if a URL or several paths)`, `Step 1 - Ensure graphify is installed` (+606 more)
...
- **Why does `FiveTurnGame` connect `FiveTurnGame` to `GameState`, `harness.py`, `prototype.py`, `engine/__init__.py`, `test_five_turn_prototype.py`, `mappers.py`, `test_balance_harness.py`, `PlayerCommand`, `InventoryState`, `cli.py`, `test_api.py`, `domain/__init__.py`?**
  _High betweenness centrality (0.045) - this node is a cross-community bridge._
- **Why does `PlayerCommand` connect `PlayerCommand` to `GameState`, `harness.py`, `prototype.py`, `test_five_turn_prototype.py`, `test_invariants.py`, `mappers.py`, `engine/__init__.py`, `test_balance_harness.py`, `FiveTurnGame`, `InventoryState`, `cli.py`, `test_api.py`, `domain/__init__.py`, `turn.py`?**
  _High betweenness centrality (0.040) - this node is a cross-community bridge._
- **Why does `resolve_turn()` connect `PlayerCommand` to `GameState`, `engine/__init__.py`, `prototype.py`, `test_determinism.py`, `test_invariants.py`, `mappers.py`, `test_five_turn_prototype.py`, `test_balance_harness.py`, `InventoryState`, `test_api.py`, `domain/__init__.py`, `turn.py`, `test_rounding.py`?**
