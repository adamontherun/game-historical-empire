...
## Corpus Check
- 88 files · ~131,354 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1241 nodes · 2171 edges · 100 communities (89 shown, 11 thin omitted)
- Extraction: 97% EXTRACTED · 3% INFERRED · 0% AMBIGUOUS · INFERRED: 64 edges (avg confidence: 0.52)
...
1. `PlayerCommand` - 96 edges
2. `resolve_turn()` - 89 edges
3. `FiveTurnGame` - 66 edges
...
- `TurnSpec` --uses--> `PressureState`  [INFERRED]
  backend/app/engine/prototype.py → backend/app/domain/pressure.py
- `test_causal_source_id_single_source()` --calls--> `PressureState`  [EXTRACTED]
  backend/tests/test_pressure_arc.py → backend/app/domain/pressure.py
- `test_pressure_stage_world_validator()` --calls--> `PressureState`  [EXTRACTED]
  backend/tests/test_pressure_arc.py → backend/app/domain/pressure.py
...
Cohesion: 0.06
Nodes (44): Engine package — deterministic RNG and rounding., next_world_known_for_turn(), pressure_for_turn(), WorldCondition, Derive world for turn idx from pressure arc., Structured threat known at turn idx (pre-turn). Only T3 (idx=2, worsening_dry)…, Return PressureState for turn idx (0..4), else raise., world_for_turn() (+36 more)
...
Cohesion: 0.10
Nodes (46): choice_map_for(), choices_for(), _completion_summary(), _market_view(), _outcome_view(), Mappers: FiveTurnGame -> GameView. Pure, no mutation., Map choice_id -> PlayerCommand for current revision., Legality + affordability ONLY (B1). No turn gating, no margin>0 gating. Two… (+38 more)
...
### Community 16 - "Section 10 — COMPLETE (2026-08-10)"
Cohesion: 0.20
Nodes (9): Boundaries, Decisions relevant, Intentionally missing, Last known green, Next milestone, Normal verification, Section 10 — COMPLETE (2026-08-10), STATE — Historical Empire (+1 more)
...
Cohesion: 0.11
Nodes (18): 001 — Scope Strategy (2026-08-09), 002 — FastAPI Conventions (2026-08-09), 003 — Tooling (2026-08-09), 004 — Testing Strategy (2026-08-09), 005 — Frontend Stack (2026-08-09), 006 — Persistence (2026-08-09), 007 — Autonomy (2026-08-09), 008 — Plan Mode (2026-08-09) — revised 2026-08-09 (+10 more)
...
Cohesion: 0.13
Nodes (36): Domain package — re-exports canonical types., Pressure domain — Section 8 world-pressure arc types. Small, frozen, validated…, GameState, InventoryState, MarketState, OperationState, PlayerState, BaseModel (+28 more)
...
Cohesion: 0.22
Nodes (17): BatchConfig, Run batch — deterministic, pure, no global random., run_batch(), Section 9 — Balance harness acceptance tests. Covers AC1-6 with confound-…, Regression: starting storage must be scarce enough that granary matters. Idle…, Matched control: policy_storage_heavy must beat itself with build_granary…, Matched control: policy_trade_heavy must beat itself with secure_route…, AC2: no universally dominant (median_ratio <1.60) and no dead (median… (+9 more)
...
Cohesion: 0.15
Nodes (27): _capital_bps(), choose_rival_command(), ObservableContext, Information visible to rivals when choosing (pre-turn)., Capital factor in bps: 0 if unaffordable, 5000 if tight, 10000 if comfortable., Integer/bps score for a given rival + command + observable context. score =…, Deterministically choose rival command via integer scoring. Rankings by…, Economic state — cash/inventory/capacities/route, no headline. (+19 more)
...
Cohesion: 0.09
Nodes (27): CausalNode, CausalTrace, model_validator, One step in the causal chain with explicit parent links., Ordered causal chain — parents appear before children, immutable tuples., Lightweight turn identity for RNG derivation., Derive TurnContext for RNG calls., TurnContext (+19 more)

### Community 91 - "turn.py"
Cohesion: 0.13
Nodes (23): DomainEffect, OutcomeDriver, PlayerOutcome, BaseModel, Causal trace, domain effects, and player outcome for Sections 4–5. Structural…, Player-visible economic effect derived from trace., Player-facing causal story grouping multiple trace nodes. Represents a distinct…, Concise player outcome for the turn reveal. (+15 more)
...
### Community 93 - "apply_rival_command"
Cohesion: 0.18
Nodes (15): Inventory value in Money at price_milli., value_for(), apply_rival_command(), Economic settlement context post player market resolution., Execute one rival's full turn via shared primitives with correct timing. Buy…, Wealth = cash + inventory value at home_price., rival_wealth(), SettlementContext (+7 more)
...
### Community 96 - "PressureState"
Cohesion: 0.16
Nodes (9): PressureState, BaseModel, model_validator, One step of the authored pressure arc — frozen, validated. Exactly 7 fields per…, BaseModel, Wealth = cash + inventory value at Home price (milli)., Concise end-of-run summary — both human and structured (now with rivals)., StrategicSummary (+1 more)

### Community 97 - "RivalProfile"
Cohesion: 0.15
Nodes (13): _exposure_bps(), _headline_for(), BaseModel, Risk factor: penalize if post-command cash would be low., Immutable preference vector — bps per command, frozen so profile cannot mutate., Exposure factor: Mira penalizes farm concentration, Daran rewards it., Derive truthful headline from reason_code and actual quantities. Partial…, Personality — preferences and risk, not economic state. (+5 more)

### Community 98 - "test_engine_purity.py"
Cohesion: 0.20
...
## Knowledge Gaps
- **556 isolated node(s):** `D1 — BLOCKING: `test_available_choices_turn_invariant` cannot detect the defect it exists for`, `D2 — Secondary`, `0.1 Purpose`, `0.2 Authority and precedence`, `0.3 Agent execution protocol` (+551 more)
...
- **Why does `resolve_turn()` connect `PlayerCommand` to `PressureState`, `engine/__init__.py`, `test_determinism.py`, `InventoryState`, `test_five_turn_prototype.py`, `test_balance_harness.py`, `test_two_markets_route.py`, `test_deterministic_rivals.py`, `test_causal_trace.py`, `turn.py`, `rivals.py`, `apply_rival_command`?**
  _High betweenness centrality (0.039) - this node is a cross-community bridge._
- **Why does `PlayerCommand` connect `PlayerCommand` to `PressureState`, `engine/__init__.py`, `RivalProfile`, `test_five_turn_prototype.py`, `InventoryState`, `test_balance_harness.py`, `test_two_markets_route.py`, `test_deterministic_rivals.py`, `FiveTurnGame`, `cli.py`, `test_causal_trace.py`, `turn.py`, `rivals.py`, `apply_rival_command`?**
  _High betweenness centrality (0.037) - this node is a cross-community bridge._
- **Why does `cost_for_quantity()` connect `rivals.py` to `RivalProfile`, `turn.py`, `mappers.py`, `test_deterministic_rivals.py`?**
...
- **Are the 12 inferred relationships involving `FiveTurnGame` (e.g. with `PressureState` and `TurnResolution`) actually correct?**
  _`FiveTurnGame` has 12 INFERRED edges - model-reasoned connections that need verification._
