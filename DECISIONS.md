...
- **Decision:** Define `MarketState.supply` as persistent regional stock at the start of the turn. Home Valley transition is `stock_next = max(0, stock + farm_output - demand)` where `demand` is regional consumption for the turn; price is set on `stock_next` (post-consumption) via the existing `_target_price` guard (`max(stock_next,1)`). River Town supply remains stable (exogenous) for Section 6. `TURN_SPECS` is a hardcoded 5-element authored arc (T1 normal high demand, T2 normal surplus weak, T3 normal warning, T4 drought, T5 normal aftermath) with truthful prose that describes actual state (surplus emergent from accumulation, not per-turn harvest differences). Determinism is via choices (`same seed+choices → same`), not via seed variation, and cross-seed "no dominant strategy" is deferred to Sections 8/9.
- **Consequence:** The 5-turn game is bounded (no accumulation hack), signals are truthful, and the three scripted policies (farm-heavy, storage-heavy, trade-heavy) are each exactly 5 legal commands and diverge. `FiveTurnGame` owns `GameState`+`history` outside the frozen `GameState`, calls `resolve_turn` once per turn with validated `rng_context`, and enforces exactly 5 submissions. CLI `backend/app/cli.py` is thin (interactive + `--choices` non-interactive).
- **Rationale:** Minimal coherent economics without spoilage or a generic DSL; keeps the kernel as authority and makes the drought → farm_output → stock → price chain inspectable.
